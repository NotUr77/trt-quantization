...
namespace model
{
    ...
    bool Model::build_engine()
    {
        ...
        auto builder = shared_ptr<IBuilder>(createInferBuilder(*m_logger));
        auto network = shared_ptr<INetworkDefinition>(builder->createNetworkV2(1));
        auto config = shared_ptr<IBuilderConfig>(builder->createBuilderConfig());
        auto parser = shared_ptr<IParser>(createParser(*network, *m_logger));
        ...
        shared_ptr<Int8EntropyCalibrator> calibrator(new Int8EntropyCalibrator(
            64,
            m_params->calibrationDataPath,
            m_params->calibrationTablePath,
            m_params->img.c * m_params->img.h * m_params->img.w, m_params->img.h, m_params->img.w));
